...
def test_compute_GL_uncorrected(partitioning):
    # def test_compute_GL_uncorrected():
    frac_pos, counts = partitioning
    # frac_pos = np.array([[1.0, 0.0, 1.0], [1.0, 1.0, 1.0]])
    # counts = np.array([[1, 2, 2], [2, 2, 2]])
    counts = filter_valid_counts(counts)
    ...
    GL3 = compute_GL_uncorrected(frac_pos, counts, psr="brier", reduce_bin=False)

    # print(GL3)
    # print(np.sum(counts, axis=1))
    # print(np.sum(counts))

    # assert not np.isnan(GL3).any()
    assert np.isfinite(GL3).all()

    n = np.sum(counts)
    if n > 0:
        GL3_reduced = np.sum(np.sum(counts, axis=1) * GL3 / n)
    else:
        GL3_reduced = 0

    # GL3_reduced = np.nan_to_num(GL3_reduced)

    # print(GL3_reduced, GL2)

    assert np.allclose(GL3_reduced, GL2)
...
def test_compute_GL_bias(partitioning):
    ...
    GL_bias2 = compute_GL_bias(frac_pos, counts, reduce_bin=False)

    assert np.isfinite(GL_bias2).all()

    n = np.sum(counts)
    if n > 0:
        GL_bias2_reduced = np.sum(np.sum(counts, axis=1) * GL_bias2 / n)
    else:
        GL_bias2_reduced = 0

    assert np.allclose(GL_bias2_reduced, GL_bias)
...
def test_compute_GL_induced(params):
    ...
    GL_induced2 = compute_GL_induced(c_hat, y_bins, reduce_bin=False)
    assert np.isfinite(GL_induced2).all()

    _, counts = np.unique(y_bins, return_counts=True)

    n = np.sum(counts)
    if n > 0:
        GL_induced2_reduced = np.sum(counts * GL_induced2 / n)
    else:
        GL_induced2_reduced = 0

    assert np.allclose(GL_induced2_reduced, GL_induced)
