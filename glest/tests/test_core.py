...
def test_partitioner_labels(data, classifier, y_scores):
    X, y = data
    partitioner = Partitioner.from_name("decision_tree")
    partitioner.fit(X, y_scores, y)
    labels = partitioner.predict(X, y_scores)
    [frac_pos_dict], counts = partitioner.evaluate_dict(y_scores, labels, [y])
    ...
    def R(X):
        y_scores = classifier.predict_proba(X)[:, 1]
        labels = partitioner.predict(X, y_scores)
        return np.array([frac_pos_dict[(label[0], label[1])] for label in labels])

    print(R(X))

    # new data
    rng = np.random.default_rng(0)
    X2 = rng.uniform(size=X.shape)
