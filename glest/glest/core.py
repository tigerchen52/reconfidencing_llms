...
class Partitioner:
    ...
    def fit(self, X, y_scores, y_true=None):
        ...
        if isinstance(self.predict_method, str) and not hasattr(
            self.estimator, self.predict_method
        ):
            raise AttributeError(
    ...
    def predict(self, X, y_scores):
        ...
        for i in range(len(self.bins_) - 1):
            ...
            if partitioner is not None and n_samples_bin > 0:
                ...
                else:  # custom way of getting the predict method
                    predict_method = self.predict_method(partitioner)
    ...
    def evaluate(self, y_scores, labels, arrays: list):
        y_bins = self.transform_bins(y_scores)
        n_bins = self.get_n_bins()

        counts = [[] for _ in range(n_bins)]
        arrays_avg = [[[] for _ in range(n_bins)] for _ in arrays]

        for i in range(n_bins):
            if self.verbose:
                print(f"Bin {i+1}/{n_bins}: evaluating.")
            bin_idx = y_bins == i
            arrays_bin = [array[bin_idx] for array in arrays]

            unique_labels, unique_counts = np.unique(
                labels[bin_idx, 1], return_counts=True
            )

            for j, array in enumerate(arrays_bin):
                for label in unique_labels:
                    if len((labels == label)[bin_idx, 1]) > 0:
                        avg = np.mean(array[(labels == label)[bin_idx, 1]])
                        arrays_avg[j][i].append(avg)

            counts[i].extend(unique_counts)

        arrays_avg = [list_list_to_array(array, fill_value=0) for array in arrays_avg]
        counts = list_list_to_array(counts, fill_value=0, dtype=int)

        return arrays_avg, counts

    def evaluate_dict(self, y_scores, labels, arrays: list):
        y_bins = self.transform_bins(y_scores)
        n_bins = self.get_n_bins()

        counts = {}
        dict_avg = [{} for _ in arrays]
        ...
        for i in range(n_bins):
            ...
            for label, count in zip(unique_labels, unique_counts):
                for j, array in enumerate(arrays_bin):
                    if len((labels == label)[bin_idx, 1]) > 0:
                        avg = np.mean(array[(labels == label)[bin_idx, 1]])
                        dict_avg[j][(i, label)] = avg

                counts[(i, label)] = count

        # dict_avg = [list_list_to_array(array, fill_value=0) for array in dict_avg]
        # counts = list_list_to_array(counts, fill_value=0, dtype=int)

        return dict_avg, counts

    @staticmethod
    def iter_region(labels):
        unique_bins = np.unique(labels[:, 0])
        for bin_id in unique_bins:
            bin_idx = labels[:, 0] == bin_id
            unique_labels = np.unique(labels[bin_idx, 1])
            for label in unique_labels:
                idx = np.logical_and(labels[:, 1] == label, labels[:, 0] == bin_id)
                yield bin_id, label, idx
    ...
    def iter_bin_from_scores(self, y_scores):
        y_bins = self.transform_bins(y_scores)
        for bin_id in range(self.get_n_bins()):
            idx = y_bins == bin_id
            yield bin_id, idx
...
class GLEstimator:
    ...
    def _evaluate(self, X, y, y_scores, partition=None):
        ...
        (frac_pos, mean_scores), counts = self.partitioner_.evaluate(
            y_scores, labels, [y, y_scores]
        )
        ...
        self.c_estimator_ = CEstimator(y_scores, y)
        self.c_hat_ = self.c_estimator_.c_hat()
    ...
    def L(self, psr: str = "brier"):
        return 2 * np.mean(np.square(self.y_scores_ - self.y_))

    def CL(self, psr: str = "brier"):
        return compute_CL(self.frac_pos_, self.counts_, self.mean_scores_, psr)

    def GL(self, psr: str = "brier", reduce_bin: bool = True):
        gl = (
            self.GL_uncorrected(psr, reduce_bin)
            - self.GL_bias(psr, reduce_bin)
            - self.GL_induced(psr, reduce_bin)
        )
        return np.clip(gl, 0, None)

    def GL_uncorrected(self, psr: str = "brier", reduce_bin: bool = True):
        if not self.is_fitted():
            raise ValueError("GLEstimator is not fitted.")

        return compute_GL_uncorrected(self.frac_pos_, self.counts_, psr, reduce_bin)

    def GL_bias(self, psr: str = "brier", reduce_bin: bool = True):
        if not self.is_fitted():
            raise ValueError("GLEstimator is not fitted.")

        return compute_GL_bias(self.frac_pos_, self.counts_, psr, reduce_bin)

    def GL_induced(self, psr: str = "brier", reduce_bin: bool = True):
        if not self.is_fitted():
            raise ValueError("GLEstimator is not fitted.")

        return compute_GL_induced(self.c_hat_, self.y_bins_, psr, reduce_bin)

    def metrics(self, psr: str = "brier", reduce_bin: bool = True):
        if not self.is_fitted():
    ...
    def __format__(self, psr: str) -> str:
        ...
        if self.is_fitted():
            ...
            metrics = self.metrics(psr, reduce_bin=True)

            extra = (
                f"  Scoring Rule      : {psr}\n"
                f"  Expected loss     : {metrics['L']:.4f}\n"
