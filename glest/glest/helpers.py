...
def compute_GL_induced(c_hat, y_bins, psr: str = "brier", reduce_bin: bool = True):
    ...
    if reduce_bin:
        return np.vdot(diff, counts) / np.sum(counts)
...
def compute_CL(frac_pos, counts, mean_scores, psr: str = "brier"):
    counts = filter_valid_counts(counts)

    prob_bins, mean_bins = calibration_curve(
        frac_pos, counts, mean_scores, remove_empty=False, return_mean_bins=True
    )

    if psr != "brier":
        raise NotImplementedError()

    diff = 2 * (np.square(prob_bins - mean_bins))
    bin_counts = np.sum(counts, axis=1)

    n_samples = np.sum(bin_counts)
    if n_samples > 0:
        return np.nansum(bin_counts * diff) / n_samples
    else:
        return 0


def compute_GL_uncorrected(
    frac_pos, counts, psr: str = "brier", reduce_bin: bool = True
):
    counts = filter_valid_counts(counts)
    ...
    if n_samples > 0:
        if not reduce_bin:
            # return np.inner(diff, counts)
            # print(counts * diff)
            # print(np.sum(counts, axis=1))
            # print(counts)
            # return np.nansum(counts * diff, axis=1) / np.sum(counts, axis=1)
            n_bins = np.sum(counts, axis=1)
            return np.divide(
                np.sum(counts * diff, axis=1),
                n_bins,
                np.full_like(n_bins, 0, dtype=float),
                where=n_bins > 0,
            )
        # diff is nan when prob_bins is nan, ie when counts in bin is 0
...
def compute_GL_bias(frac_pos, counts, psr: str = "brier", reduce_bin: bool = True):
    if psr != "brier":
