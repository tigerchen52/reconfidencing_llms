...
parser.add_argument('-checker_device', type=str, default='cpu')
...
def get_confidence_by_just_asking(model, checker, prompt, sample_num, device, temperature=1.0, max_new_tokens=500, do_sample=True):
    answer, confidence = checker.calculate_confidence(prompt, sample_num, temperature, do_sample=do_sample)
...
def run(model, checker, data_file, out_file, prompt, sample_num, device, temperature=1.0, max_new_tokens=500, do_sample=True):
    ...
    for index, line in enumerate(open(data_file, encoding="utf8")):
        ...
        if args.confidence_checker == "just_ask_for_calibration":
            answer, avg_confidence = get_confidence_by_just_asking(model, checker, temp_prompt, sample_num, device, temperature=temperature, max_new_tokens=max_new_tokens, do_sample=do_sample)
        else:
        ...
        print("index = {a}, name = {b}, answer = {c}".format(a=index, b=name, c=answer))
        obj = {
            "name":name,
            "backlink":backlink,
            "answer":answer,
...
if __name__ == '__main__':
    ...
    prompt = RELATION_PROMPTS[relation]
    model_name = args.model_name_on_hg
    model = LLM[args.llm](model_name)
    checker = CHECKER[args.confidence_checker](device=checker_device)
